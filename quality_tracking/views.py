...
def personnel_detail(request, user_id):
    """Bir personelin puantaj, üretim hareketleri ve hata kayıtlarını tek kronolojide gösterir."""
    ...
    production_events = (
        OrderEvent.objects
        .filter(
            Q(user=target_user.username) | Q(ortak_calisanlar__icontains=target_user.username),
    ...
    issues = (
        ...
        .filter(
            sorumlu_personeller=target_user,
            created_at__gte=start_dt,
            created_at__lt=end_dt,
        )
        .select_related("order", "order__musteri", "kaydeden")
        .prefetch_related("sorumlu_personeller")
        .distinct()
        .order_by("-created_at")
    )
    ...
    attendance = (
        ...
        .filter(user=target_user, work_date__range=(start_date, end_date))
        .order_by("-work_date")
    )
    ...
    for record in attendance:
        if record.check_in:
            timeline.append({
                ...
                "kind": "attendance_in",
                "title": "İşe giriş",
                "detail": record.get_status_display(),
                "attendance": record,
            })
        if record.check_out:
            timeline.append({
    ...
    for event in production_events:
        ...
        timeline.append({
            ...
            "kind": "production",
            "title": f"{event.stage} → {event.value}",
            "detail": event.aciklama or ("Ortak çalışan olarak kayıtlı" if shared else ""),
            "event": event,
            "order": event.order,
            "shared": shared,
        })
    ...
    for issue in issues:
        timeline.append({
            ...
            "kind": "issue",
            "title": f"Hata / Şikayet: {issue.konu}",
            "detail": issue.aciklama,
            "issue": issue,
            "order": issue.order,
        })
    ...
    return render(request, "quality_tracking/personnel_detail.html", {
