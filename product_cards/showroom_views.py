...
def _serialize_draft(draft):
    ...
    return {"ok":True,"draft":draft.id,"customer_id":str(draft.customer_id or ""),"items":groups,"discount_rate":str(draft.discount_rate or 0),"discount_amount":str(draft.overall_discount_amount or 0),"updated_at":draft.updated_at.isoformat() if draft.updated_at else None}
...
def showroom_draft_autosave(request):
    ...
    if not isinstance(raw_items,list): return JsonResponse({"ok":False,"message":"Ürün verisi geçersiz."},status=400)
