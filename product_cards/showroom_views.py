...
def _draft_summary(draft):
    ...
    discount=min(subtotal,max(Decimal("0"),discount)); taxable=max(Decimal("0"),subtotal-discount); vat_rate=max(Decimal("0"),min(Decimal("100"),draft.vat_rate or Decimal("0"))); vat=taxable*vat_rate/Decimal("100"); total=taxable+vat
    ...
    return {"id":draft.id,"customer":draft.customer.ad if draft.customer else "Müşteri seçilmedi","product_count":items["product_count"] or 0,"total_qty":items["total_qty"] or 0,"subtotal":str(subtotal.quantize(Decimal("0.01"))),"discount":str(discount.quantize(Decimal("0.01"))),"vat_rate":str(vat_rate.quantize(Decimal("0.01"))),"vat":str(vat.quantize(Decimal("0.01"))),"total":str(total.quantize(Decimal("0.01"))),"collected":str(collected.quantize(Decimal("0.01"))),"promised":str(promised.quantize(Decimal("0.01"))),"remaining":str(remaining.quantize(Decimal("0.01"))),"updated_at":timezone.localtime(draft.updated_at).strftime("%d.%m.%Y %H:%M"),"status":draft.status}
...
def showroom_edit_save(request,draft_id):
    ...
    if validation_error:
        return JsonResponse({"ok":False,"message":validation_error},status=400)
    rate=max(Decimal("0"),min(Decimal("100"),_decimal(payload.get("discount_rate"),"0"))); amount=max(Decimal("0"),_decimal(payload.get("discount_amount"),"0")); vat_rate=max(Decimal("0"),min(Decimal("100"),_decimal(payload.get("vat_rate"),"0"))); create_rows=[]
    for item in raw_items:
