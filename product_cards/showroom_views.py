...
def _serialize_draft(draft):
    if not draft:
        return {"ok": True, "draft": None, "customer_id": "", "order_taken_by": "", "items": [], "payments": [], "discount_rate": "0", "discount_amount": "0", "vat_rate": "0", "previous_balance": "0"}
...
def showroom_draft_autosave(request):
    ...
    with transaction.atomic():
        ...
        draft=_active_draft(request.user) or _create_draft(request.user,customer); draft.customer=customer; draft.order_taken_by=order_taken_by; draft.vat_rate=vat_rate; draft.previous_balance=previous_balance; draft.save(update_fields=["customer","order_taken_by","vat_rate","previous_balance","updated_at"]); draft.items.all().delete(); create_rows=[]
        for item in raw_items:
