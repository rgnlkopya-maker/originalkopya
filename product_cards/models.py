...
def consume_materials_on_stage_completion(sender,instance,created,**kwargs):
    ...
    with transaction.atomic():
        ...
        for material,qty in locked:
            before=material.stok_miktari;after=before-qty;material.stok_miktari=after;material.save(update_fields=["stok_miktari","updated_at"]);MaterialStockMovement.objects.create(material=material,movement_type="URETIM",miktar=qty,onceki_stok=before,sonraki_stok=after,aciklama=f"{instance.order.siparis_numarasi} {stage_label} reçetesi tüketimi",order=instance.order,source_event=instance)
...
def restore_materials_when_stage_deleted(sender,instance,**kwargs):
    ...
    with transaction.atomic():
        ...
        for movement in movements:
            material=Material.objects.select_for_update().get(pk=movement.material_id);before=material.stok_miktari;after=before+movement.miktar;material.stok_miktari=after;material.save(update_fields=["stok_miktari","updated_at"]);MaterialStockMovement.objects.create(material=material,movement_type="URETIM_IADE",miktar=movement.miktar,onceki_stok=before,sonraki_stok=after,aciklama=f"{instance.order.siparis_numarasi} {stage_label} kaydı silindi; stok iade edildi",order=instance.order);movement.reversed=True;movement.save(update_fields=["reversed"])
