...
STAGE_TRANSLATIONS = {
    ...
    # Eski kayıtların okunabilir kalması için backend desteği korunur.
...
def order_list(request):
    """Order List'in mevcut görünümünü koruyup güçlü AND + çoklu OR filtreleme uygular."""
    ...
    new_flags = {
        o.id: (seen_map.get(o.id) is None or o.last_updated > seen_map[o.id])
        for o in all_orders
    }
    ...
    # Aynı başlıktaki seçimler OR (__in), farklı başlıklar AND olarak birleşir.
    multi_filters = {
        "siparis_numarasi__in": _multi(request, "siparis_no"),
        "musteri__ad__in": _multi(request, "musteri"),
        "urun_kodu__in": _multi(request, "urun_kodu"),
        "urun_tipi__in": _multi(request, "urun_tipi"),
        "renk__in": _multi(request, "renk"),
        "beden__in": _multi(request, "beden"),
        "siparis_tipi__in": _multi(request, "siparis_tipi"),
        "musteri_referans__in": _multi(request, "musteri_referans"),
    }
    ...
    if status_filter:
        ...
        for pair, label in STAGE_TRANSLATIONS.items():
            if label in status_filter:
                status_q |= Q(latest_stage=pair[0], latest_value=pair[1])
        qs = qs.filter(status_q)

    siparis_baslangic = request.GET.get("siparis_tarihi_baslangic", "").strip()
    ...
    if siparis_bitis:
        qs = qs.filter(siparis_tarihi__lte=siparis_bitis)
    if teslim_baslangic:
    ...
    if kalite == "acik":
        qs = qs.filter(quality_issues__durum="ACIK").distinct()
    elif kalite == "var":
        qs = qs.filter(quality_issues__isnull=False).distinct()
    elif kalite == "yok":
        qs = qs.filter(quality_issues__isnull=True)

    aktif_count = base_qs.filter(is_active=True).count()
    pasif_count = base_qs.filter(is_active=False).count()
    sevke_count = base_qs.filter(is_active=True, latest_stage="sevkiyat_durum", latest_value="gonderildi").count()
    ...
    for order in page_obj:
        ...
        if key in STAGE_TRANSLATIONS:
            ...
            nice_stage = (order.latest_stage.replace("_durum", "").replace("_fason_durumu", " Fason")
                          .replace("_", " ").title())
    ...
    context = {
        ...
        "musteri_referans_options": (Order.objects.exclude(musteri_referans__isnull=True).exclude(musteri_referans__exact="")
                                    .values_list("musteri_referans", flat=True).distinct().order_by("musteri_referans")),
        "status_options": sorted(set(STAGE_TRANSLATIONS.values())),
        "siparis_tipi_options": Order.SIPARIS_TIPLERI,
