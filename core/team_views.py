...
def _service_parts(start, today):
    ...
    anchor_year=start.year+years; anchor=date(anchor_year,start.month,min(start.day,monthrange(anchor_year,start.month)[1])); months=0; cursor=anchor
    while True:
...
def _annual_leave_entitlement(start,as_of):
    ...
    years,_,_=_service_parts(start,as_of);return years*14

def _selected_range(request,today,employment_start=None,employment_end=None):
    effective_today=min(today,employment_end) if employment_end else today; preset=request.GET.get("preset","this_month"); start_raw=request.GET.get("start","").strip(); end_raw=request.GET.get("end","").strip()
    if start_raw or end_raw:
        try:
            start=date.fromisoformat(start_raw) if start_raw else effective_today; end=date.fromisoformat(end_raw) if end_raw else effective_today
...
def user_management_view(request):
    ...
    if request.method=="POST":
        ...
        if action=="create_user":
            username=request.POST.get("username","").strip();password=request.POST.get("password","").strip();role=request.POST.get("role","").strip();gorev=request.POST.get("gorev","yok").strip();first_name=request.POST.get("first_name","").strip();last_name=request.POST.get("last_name","").strip()
...
def employee_detail(request,user_id):
    ...
    if request.method=="POST":
        ...
        try:
            ...
            if User.objects.exclude(pk=employee.pk).filter(username__iexact=username).exists():messages.error(request,"Bu kullanıcı adı başka bir kullanıcı tarafından kullanılıyor.");return redirect("employee_detail",user_id=employee.id)
            employee.username=username;employee.first_name=request.POST.get("first_name","").strip();employee.last_name=request.POST.get("last_name","").strip();new_password=request.POST.get("new_password","").strip()
            if new_password:employee.set_password(new_password)
            employee.save();role=request.POST.get("role","personel");role=role if role in {"personel","mudur","patron"} else "personel";employee.groups.clear();group,_=Group.objects.get_or_create(name=role);employee.groups.add(group);gorev=request.POST.get("gorev","yok");valid_gorevler={value for value,_label in TEAM_CHOICES};user_profile.gorev=gorev if gorev in valid_gorevler else "yok";user_profile.save(update_fields=["gorev"])
    ...
    issue_qs=QualityIssue.objects.filter(sorumlu_personeller=employee,created_at__date__range=(range_start,range_end)).select_related("order","order__musteri","kaydeden").distinct().order_by("-created_at","-id");issue_count=issue_qs.count();open_issue_count=issue_qs.filter(durum="ACIK").count()
    if issue_count:operation_counts.append({"stage":"quality_issue","value":"issue","label":"⚠️ Hata Kaydı","count":issue_count})
    activity_items=[]
...
def my_page(request):
    ...
    range_start, range_end, preset = _selected_range(
        request, today, profile.employment_start_date, profile.employment_end_date
    )

    range_records = AttendanceRecord.objects.filter(
        user=employee, work_date__range=(range_start, range_end)
    )
    ...
    service_years, service_months, service_days = _service_parts(
        profile.employment_start_date, range_end
    )

    today_record = AttendanceRecord.objects.filter(user=employee, work_date=today).first()

    work_events_qs = (
        OrderEvent.objects.select_related("order", "order__musteri")
        .filter(
    ...
    operation_counts = [
        {
            "stage": row["stage"],
            "value": row["value"],
            "label": _event_label(row["stage"], row["value"]),
            "count": row["count"],
        }
    ...
    for issue in issue_qs:
        activity_items.append(SimpleNamespace(
            ...
            operation_label=f"Hata: {issue.konu} · {'Açık' if issue.durum == 'ACIK' else 'Çözüldü'}",
            order=issue.order,
    ...
    work_events_page = Paginator(activity_items, 50).get_page(request.GET.get("work_page"))
    ...
    team_label = dict(TEAM_CHOICES).get(
        user_profile.gorev, user_profile.gorev.title()
    )

    return render(request, "teams/my_page.html", {
        "employee": employee,
        "profile": profile,
        ...
        "work_events_total": work_events_total,
        "work_events_page": work_events_page,
